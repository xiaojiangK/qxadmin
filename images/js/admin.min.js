...
window.onload = function (){
    (function (){
        ...
        // button
        // 还未区分
        if ($('qx-button-group').length > 0) {
            $('qx-button-group').each(function (index){
                $(this).wrapAll('<div class="qx-button-group"></div>');
                var button = '';
                $(this).find('qx-button').each(function (){
                    ...
                    const disabled = $(this).attr('disabled');
                    button+=`
                        <button type="${native ? native : 'button'}" class="${type ? type+' ' : ''}${size ? size+' ' : ''}${disabled ? 'is-disabled ' : ''}qx-button"><i class="iconfont ${icon ? icon : ''}"></i><span>${$(this).text()}</span></button>
                    `;
                });
                $('.qx-button-group').eq(index).html(button);
            });
        } else {
            $('qx-button').each(function (index){
                $(this).wrapAll('<div class="qx-button"></div>');
                var button = '';
                const type = $(this).attr('type');
                const size = $(this).attr('size');
                const icon = $(this).attr('icon');
                ...
                button+=`
                    <button type="${native ? native : 'button'}" class="${type ? type+' ' : ''}${size ? size+' ' : ''}${disabled ? 'is-disabled ' : ''}qx-button"><i class="iconfont ${icon ? icon : ''}"></i><span>${$(this).text()}</span></button>
                `;
                $('.qx-button').eq(index).html(button);
            });
        }
        $('body').button();
    })(window);
    ...
    $.fn.checkbox = function (option){
        ...
        $(this).find('.qx-checkbox').each(function (){
            $(this).off().on('click', function (){
                ...
                // 禁用状态
                if (hasClass(checkbox, 'is-disabled')) {
                    return;
                }
                ...
                if (hasClass(checkbox, 'is-checked')) {
                    $(this).removeAttr('aria-checked');
                    $(checkbox).removeClass('is-checked');
                    global.checked[gIdx] = global.checked[gIdx].filter(item => item.id != checkIdx);
                } else {
                    $(this).attr('aria-checked', true);
                    $(checkbox).addClass('is-checked');
                    global.checked[gIdx].push({
                        id: checkIdx,
                        value
                    });
                }
                if (opts.change) {
                    opts.change(global.checked[gIdx]);
                }
                return false;
            });
        });
        ...
        $(this).find('.qx-select').each(function (){
            $(this).off().on('click', function (e){
                ...
                $(this).find('.qx-select_item').each(function (){
                    $(this).off().on('click', function (){
                        // 可多选
                        if (hasClass(that, 'is-multiple')) {
                            const id = $(this).attr('data-idx');
                            if (hasClass(this, 'active')) {
                                $(this).removeClass('active');
                                global.selectd = global.selectd.filter(item => item.id != id);
                            } else {
                            ...
                            let value = '';
                            global.selectd.map((item, index) => {
                                if (index == global.selectd.length - 1) {
                                    value += `${item.value}`;
                                } else {
                                    value += `${item.value} / `;
                                }
