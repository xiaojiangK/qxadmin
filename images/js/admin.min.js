...
;(function($){
    ...
    $.fn.input = function (option){
        ...
        var opts = $.extend(config, option);
        $(this).on('click', '.iconclose-', function (){
            $(this).parents('.qx-input_box').find('input').val('');
            return false;
        });
        $(this).on('click', '.iconsee', function (){
    ...
    $.fn.DatePicker = function (option){
        ...
        $(this).find('.c-datepicker-date-editor').datePicker({
            min: opts.min,
            max: opts.max,
            format: opts.format,
    ...
    $.fn.tabs = function (option){
        ...
        if (opts.addItem) {
            ...
            $(pack).find('.qx-tabs_nav').append(`
                <div class="qx-tabs_item" data-idx="${g.itemLen++}">${title} ${isClose ? `<span class="iconfont iconclose"></span>` : ''}</div>
            `);
    ...
    $.fn.message = function (option){
        ...
        $('body').append(`
            <div role="alert" style="transform: translate3d(-50%, ${65 * length}px, 0)" class="qx-message qx-message-${opts.type} ${opts.showClose ? 'is-closable' : ''}">
                <i class="iconfont iconmsg-${opts.type}"></i>
                <p class="qx-message_con">${opts.msg}</p>
                ${opts.showClose ? `<i class="iconfont iconclose"></i>` : ''}
            </div>
        `);
...
(function (){
    ...
    $('qx-form').each(function (index){
        ...
        $(this).find('qx-form-item').each(function (){
            ...
            const required = $(this).attr('required');
            formItem+=`
                <div class="${required ? 'required ' : ''}qx-form-item">
        ...
        $('.qx-form').eq(index).find('.qx-form-item').each(function (index){
            const that = this;
            if (t.hasClass(this, 'required')) {
                const con = $(this).find('.qx-form_con');
                const text = $(that).find('.qx-form_label').text();
                $(this).find('input').on('input', function (){
                    const input = $(that).find('.qx-input');
                    $(that).find('.qx-form_error').remove();
                    if (!$(this).val()) {
                        $(input).addClass('error');
                        $(con).append(`<div class="qx-form_error">请输入${text}</div>`);
                    } else {
                        $(input).removeClass('error');
                    }
                });
                $(this).find('textarea').on('input', function (){
                    const textarea = $(that).find('.qx-textarea');
                    $(that).find('.qx-form_error').remove();
                    if (!$(this).val()) {
