...
;(function($){
    ...
    $.fn.input = function (option){
        ...
        $(this).on('click', '.icon-closes', function (){
            $(this).parents('.qx-input').find('input').val('');
            return false;
        });
        $(this).on('click', '.icon-see', function (){
            var input = $(this).parents('.qx-input').find('input');
    ...
    $.fn.createPage = function(option) {
        ...
        zp.init($(this).find('.pagination'), page);
    }
        ...
        $(this).off().on('click', '.qx-tabs_item', function (){
            ...
            $(pane).each(function (){
                if ($(this).attr('data-idx') == index) {
                    t.setFrameSrc($(this).find('iframe'));
                    $(this).addClass('active').siblings().removeClass('active');
                }
            });
                ...
                if (next.length) {
                    ...
                    $(pane).each(function (){
                        if ($(this).attr('data-idx') == index) {
                            ...
                            t.setFrameSrc($(this).next().find('iframe'));
                        }
                    });
                } else {
                    ...
                    $(pane).each(function (){
                        if ($(this).attr('data-idx') == index) {
                            ...
                            t.setFrameSrc($(this).prev().find('iframe'));
                        }
                    });
                }
            }
    ...
    $.fn.layer = function (option){
        ...
        if (opts.resize) {
            $('.qx-layer').mousedown(function (e){
                var e = e || window.event;
                var layer = $(this);
                var resize = $(this).find('.resize');
                var disX = e.pageX - $(resize).position().left;
                var disY = e.pageY - $(resize).position().top;
                $(document).mousemove(function (e){
                    var e = e || window.event;
                    var l = e.pageX - disX + $(resize).outerWidth();
                    var t = e.pageY - disY + $(resize).outerHeight();
                    if (l <= 300) {
                        l = 300;
                    }
                    if (t <= 300) {
                    ...
                    $(layer).find('iframe').css('height', t - 42);
                    return false;
                });
                $(document).on('mouseup mouseleave', function() {
                    $(document).off('mousemove mouseup');
                ...
                (function(i){
                    $li.click(function(){
                        ...
                        if (!data[i].children) {
                            ...
                            var cascader = $(that).find('.cascader-menu');
    ...
    $.fn.table = function(options) {
        ...
        var datas = opts.data;
        var checkAll = function(len1, len2, all) {
            ...
            $(`#${id}`).on('click', 'td' ,function (){
                if($(this).attr('data-cid')) {
                    ...
                    for (let i of $(`#${id} td`)) {
                        ...
                        var cpid = $(i).attr('data-pid');
                        if (csid && cpid && !pid) {
                            for (let j of $(`#${id} td`)) {
                                if (csid == $(j).attr('data-pid')) {
                                    if (hasClass) {
                                        $(j).slideUp('fast');
                                        $(i).find('.iconfont').removeClass('active');
                                    }
                                }
                            }
                        }
                ...
                for (let i of td) {
                    ...
                    if (cid) {
                        $(checks).each(function(i, item) {
                            var pid = $(item).parents('td').attr('data-pid');
                            if (!curid) {
                                for (let i of $(item).parents('tr').find('td')) {
                                    var csid = $(i).attr('data-cid');
                                    var cpid = $(i).attr('data-pid');
                                    if (csid && cid == cpid) {
...
;(function (){
    ...
    $('qx-cascader').each(function (){
        ...
        <div class="qx-cascader">
            <div class="qx-select ${size ? `${size} ` : ''}qx-input">
    ...
    $('qx-button').each(function (){
        ...
        var round = t.getAttr(this, 'round');
        var upload = t.getAttr(this, 'upload');
    ...
    setTimeout(() => {
        ...
        if ($('qx-radio').length > 0) {
            $('qx-radio').each(function (index) {
                ...
                var button = t.getAttr(this, 'button');
                var bordered = t.getAttr(this, 'bordered');
        ...
        if ($('qx-select').length > 0) {
            $('qx-select').each(function (index){
                ...
                var place = $(this).attr('placeholder');
                var disabled = $(this).attr('disabled');
                var iconGroup = t.getAttr(this, 'icon-group');
                var filter = t.getAttr(this, 'filterable');
                var multi = t.getAttr(this, 'multiple');
        ...
        if ($('qx-switch').length > 0) {
            $('qx-switch').each(function (index){
                ...
                var checked = $(this).attr('checked');
                var disabled = $(this).attr('disabled');
                var activeTxt = $(this).attr('active-text');
                var inactiveTxt = $(this).attr('inactive-text');
                var activeVal = $(this).attr('active-value');
                var inactiveVal = $(this).attr('inactive-value');
        ...
        if ($('qx-input').length > 0) {
            $('qx-input').each(function (){
                ...
                var place = $(this).attr('placeholder');
                var auto = $(this).attr('autocomplete');
                var showWord = t.getAttr(this, 'show-word');
                var showPass = t.getAttr(this, 'show-pass');
                var clear = t.getAttr(this, 'clearable');
                if (type == 'textarea') {
                    $(this).wrapAll(`
    ...
    $('qx-pagination').each(function (){
        ...
        var small = t.getAttr(this, 'small');
    ...
    if ($('qx-tabs').length > 0) {
        $('qx-tabs').each(function (){
            ...
            var tabs = '';
            var type = $(this).attr('type');
            var closable = t.getAttr(this, 'closable');
    ...
    $('.qx-top .toggle').hover(function (){
        $(this).find('.nav-child').stop().fadeIn('fast').clearQueue().animate({
            top: '48px'
        }, 'fast');
        $(this).find('.icon-icondown').css('transform', 'rotate(180deg)');
    }, function (){
        $(this).find('.nav-child').stop().fadeOut('fast').clearQueue().animate({
            top: '65px'
        });
    ...
    if (pane.length == 1) {
        t.setFrameSrc($(pane).find('iframe').eq(0));
    }
    // tabs init
