...
const g = {
    ...
    itemLen: 0,
    paneLen: 0,
    icon: "icon-info,icon-infos,icon-success,icon-successs,icon-hook,icon-error,icon-hollow-close,icon-warning,icon-warnings,icon-fileopen,icon-retract,icon-see,icon-sees,icon-max,icon-refresh,icon-ref-left,icon-ref-right,icon-arrowright,icon-arrowleft,icon-arrowdown,icon-arrowup,icon-left-round,icon-right-round,icon-magnifier,icon-uploadpic,icon-others,icon-notice,icon-adjust,icon-data,icon-form,icon-navigation,icon-share,icon-shares,icon-loading,icon-spinner,icon-uploads,icon-upload,icon-forks,icon-close,icon-closes,icon-seekicon,icon-searchs,icon-payconfig,icon-coin,icon-export,icon-daochu,icon-weChat,icon-alipay,icon-icondown,icon-iconup,icon-users,icon-user,icon-resets,icon-reset,icon-edits,icon-edit,icon-password,icon-passwd,icon-remove,icon-delete,icon-adds,icon-add,icon-menu,icon-menuopen,icon-finance,icon-wallet,icon-custom,icon-customs,icon-fork,icon-fork-spoon,icon-square-full,icon-goblet-full,icon-cream-square,icon-goblet,icon-cup,icon-hot-water,icon-cream-round,icon-potato-strips,icon-cold-drink,icon-cloudy-sunny,icon-aid-kit,icon-off-microphone,icon-chat-round,icon-chatline-round,icon-chatdot-round,icon-chatdot-square,icon-takeaway-box,icon-docs,icon-cilun,icon-eb,icon-thunder,icon-switchbtn,icon-coffee,icon-home-normal,icon-home,icon-cream,icon-tickets,icon-web,icon-headset,icon-chilun,icon-hollowbuy,icon-buy,icon-cooperate,icon-plus,icon-aircraft,icon-fullmore,icon-more,icon-help,icon-outlinehelp,icon-female,icon-male,icon-play,icon-platform,icon-outlinereduce,icon-qrcode,icon-picture,icon-mingfood,icon-message,icon-refrigerator,icon-alarmclock,icon-alarmclocks,icon-mouse,icon-goods,icon-dish,icon-doc,icon-bicycle,icon-scan,icon-printing,icon-tools,icon-toolss,icon-toolsbox,icon-toolsboxs,icon-clip,icon-place,icon-places,icon-cloudy,icon-messages,icon-mobile,icon-mobiles,icon-link,icon-trophy,icon-sunny,icon-shangji,icon-truck,icon-trophystar,icon-clasphands,icon-lollipop,icon-watch,icon-watchs,icon-plane,icon-pause,icon-pictures,icon-camera,icon-scissors,icon-doubleright,icon-doubleleft,icon-minus,icon-cameras,icon-phone,icon-phonecall,icon-goodss,icon-car,icon-stars,icon-star,icon-videos,icon-video,icon-voice,icon-check,icon-umbrella",
...
;(function($){
            ...
            $(this).find('.qx-select_item').each(function (){
                $(this).off().on('click', function (){
                    ...
                    if ($(this).parents('.icon').length) {
                        $(input).parent().find('.iconfont').remove();
    ...
    $.fn.tabs = function (option){
        ...
        if (opts.addItem) {
            ...
            $(pack).find('.qx-tabs_nav').append(`
                <div class="qx-tabs_item active" data-idx="${g.itemLen++}">${title} ${isClose ? `<span class="iconfont icon-forks"></span>` : ''}</div>
            `);
            $(pack).find('.qx-tabs_con').append(`
                <div class="qx-tab-pane active" data-idx="${g.paneLen++}">${content}</div>
            `);
        }
    }
                    ...
                    <div class="qx-msg_content">
                        ...
                        ${opts.showInput ? `
                            <div class="qx-msg_input">
                                <div class="qx-input">
                                    <input type="${opts.inputType ? opts.inputType : 'text'}" autocomplete="off" ${opts.inputValue ? `value=${opts.inputValue}` : ''} ${opts.placeholder ? `placeholder=${opts.placeholder}` : ''} ${opts.maxLength ? `maxLength=${opts.maxLength}` : ''} ${opts.minLength ? `minLength=${opts.minLength}` : ''} class="qx-input_inner" />
                                </div>
                            </div>
                        ` : ''}
                    </div>
    ...
    $.fn.table = function(options) {
        var defaults = {
            ...
            checked: null,              // 复选框监听事件
            switch: null,               // switch监听事件
            event: null                   // 自定义事件对象
        }
        var that = this;
        ...
        var init = function(dom) {
            ...
            function(i, item) {
                ...
                function(j, item2) {
                    ...
                    var defaults = opts.defaultClose;   // 默认折叠
                    if (item2.width) {
                        tdw = `width=${item2.width}px`;
                    }
                    if (item2.checkBox) {
                        if (collapse && pid > 0) {
                            arrB.push(`<td ${pid ? `data-pid=${pid}` : ''} class="${defaults ? 'none' : ''}" ${tdw}><span class='checkboxs'><input type='checkbox' name='Checks' value='${item[item2.keyName]}'/></span></td"">`);
                        } else {
    ...
    $.fn.closeLayer = function (){
        var layer = $(top.window.document.getElementsByClassName('qx-layer'));
...
;(function (){
        ...
        $('qx-select').each(function (index){
            ...
            if ($(this).find('qx-option').length) {
                $(this).find('qx-option').each(function (index){
                    ...
                    if (!iconGroup) {
                        options += `<li class="${checked ? 'active ' : ''}qx-select_item" data-idx="${index}">${itemVal}<span class="hook iconfont icon-hook"></span></li>`;
                    }
                });
                empty = '';
            } else {
            ...
            if (iconGroup) {
                var icon = g.icon.split(',');
                for (let i = 0; i < icon.length; i++) {
                    options += `<li class="qx-select_item" data-idx="${i}"><i class="iconfont ${icon[i]}"></i></li>`;
                }
                empty = '';
            }
            $('.qx-select').eq(index).html(`
                <div class="qx-input ${size ? `${size} qx-input_box` : ''}">
                    <input type="text" ${!filterable ? 'readonly' : ''} name="${name ? name : ''}" value="${value}" autocomplete="off" ${place ? `placeholder="${place}"` : ''} class="qx-input_inner" />
                    <span class="qx-input_suffix">
    ...
    // qx-navmenu
    $('.qx-navmenu #nav a').each(function (){
        if (t.hasClass(this, 'active')) {
    ...
    $('.qx-navmenu .sub-menu a').each(function (){
        if (t.hasClass(this, 'active')) {
